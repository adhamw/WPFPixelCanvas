...
namespace WPFPixelCanvas.Canvas.Models.Boids
{
    ...
    public class BoidSimulator
    {
        ...
        //## Constructor(s)
        public BoidSimulator(int screenWidth, int screenHeight)
        {
            //Init private fields
            _randomSource = new Random();

            // Define limits
            BoidLimits.AccelerationMin = new Vector2D(-2.7, -2.7);
            BoidLimits.AccelerationMax = new Vector2D(2.7, 2.7);
            BoidLimits.VelocityMin = new Vector2D(-20.1, -20.1);
            BoidLimits.VelocityMax = new Vector2D(20.1, 20.1);
            BoidLimits.PositionsMin = new Vector2D(0.0, 0.0);
            BoidLimits.PositionsMax = new Vector2D(screenWidth - 1, screenHeight - 1);
        ...
        {
            // Create the boids
            _boids = new Boid[NumberOfBoids];           // Creates array with space for all boids
            ...
            for (int i = 0; i < NumberOfBoids; i++)
            {
                ...
                _boids[i] = new Boid(_randomSource, i + 1)
                {
            ...
            // Set up leader boids. 
            List<Boid> leaderboids = new List<Boid>();
        ...
        public Boid[] Boids { get { return _boids; } }                              // All our boids
